        ...
        <el-card class="towcard threecard">
          ...
          <el-col class="addBtnBox">
            <el-col :span="24">
              ...
              <el-select v-model="staticTypeVal" style="width: 180px">
                <el-option
              ...
              <label style="margin: 0 15px">产品类型</label>
              <el-select v-model="productTypeVal" style="width: 180px">
                <el-option
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <span>{{ scope.row.staticType === 1 ? '在线检测' : 'API接口' }}</span>
              </template>
            </el-table-column>
            <el-table-column
              prop="productType"
...
export default {
  ...
  methods: {
    ...
    async getConsumePage() {
      ...
      params.append('productType', this.productTypeVal)
      params.append('page', page)
      params.append('size', size)
      const { data } = await this.$http.post(
        'front/dailyInfo/list/direct',
        params
      )
      if (data.code !== 200) return this.$message.error(data.msg)
