  ...
  <div class="empty">
    <el-row :span="24">
      <div class="product-title">
        <div class="i-title"><img src="../assets/img/emptynum.png">
        ...
        <el-card class="towcard">
          ...
          <el-row>
            <el-popover placement="right" width="160" trigger="hover">
              <div>
                <p>txt: 至少3001-150w条</p>
              </div>
              <div class="upload" slot="reference">
                <div class="upload_div">请上传TXT文本文档格式的文件进行检测（最低3001—150W条号码）</div>
                <uploader
                  ref="uploader"
                  class="upload-demo"
                  :options="options"
          ...
          <p class="gray">注:文件格式支持txt（每行一个手机号）/文件大小不超过20MB/最多上传30个文件（多文件上传必须保证所选文件在同一文件夹下面并一次性选择上传）</p>
        </el-card>
      </el-col>
    </el-row>
    <!-- 检测结果 -->
            ...
            <el-col :span="6">
              <div class="result-box">
                <p class="result-title">
                  实号包
                  <el-popover placement="right" width="160" trigger="hover" content="活跃用户">
                    <i class="iconfont iconquestion icon" slot="reference"></i>
                  </el-popover>
                </p>
                <p class="result-size">{{ latestEmpty.realCount }}</p>
                <!-- <p class="result-mb">大小: 0</p> -->
                <el-button size="medium" :disabled="!latestEmpty.realUrl" @click="downloadPro(latestEmpty.realUrl)">下载</el-button>
              </div>
            </el-col>
            <el-col :span="6">
              <div class="result-box">
                <p class="result-title">
                  沉默包
                  <el-popover placement="right" width="160" trigger="hover" content="超过六个月未激活的空号，近三个月平均流量低于30M的用建议当成无效处理。">
                    <i class="iconfont iconquestion icon" slot="reference"></i>
                  </el-popover>
                </p>
                <p class="result-size">{{ latestEmpty.silentCount }}</p>
                <!-- <p class="result-mb">大小: 0</p> -->
                <el-button size="medium" :disabled="!latestEmpty.silentUrl" @click="downloadPro(latestEmpty.silentUrl)">下载</el-button>
              </div>
            </el-col>
            <el-col :span="6">
              <div class="result-box">
                <p class="result-title">
                  空号包
                  <el-popover placement="right" width="160" trigger="hover" content="停机空号">
                    <i class="iconfont iconquestion icon" slot="reference"></i>
                  </el-popover>
                </p>
                <p class="result-size">{{ latestEmpty.emptyCount }}</p>
                <!-- <p class="result-mb">大小: 0</p> -->
                <el-button size="medium" :disabled="!latestEmpty.emptyUrl" @click="downloadPro(latestEmpty.emptyUrl)">下载</el-button>
              </div>
            </el-col>
            <el-col :span="6">
              <div class="result-box" style="border-right:0;">
                <p class="result-title">
                  风险包
                  <el-popover placement="right" width="160" trigger="hover" content="长时间关机或未开通语音服务以及易投诉的用户">
                    <i class="iconfont iconquestion icon" slot="reference"></i>
                  </el-popover>
                </p>
                <p class="result-size">{{ latestEmpty.riskCount }}</p>
                <!-- <p class="result-mb">大小: 0</p> -->
                <el-button size="medium" :disabled="!latestEmpty.riskUrl" @click="downloadPro(latestEmpty.riskUrl)">下载</el-button>
              </div>
            </el-col>
          </el-row>
          <el-row :span="24">
            <el-col :span="6" :offset="9" class="downloadAll">
              <p class="solid"></p>
              <el-button size="medium" :disabled="!latestEmpty" @click="downloadTab(tableData[0])">下载全部</el-button>
            </el-col>
          </el-row>
        </el-card>
      </el-col>
      ...
      <el-col :span="24">
        <el-card class="towcard threecard">
          ...
          <el-col class="addBtnBox">
            ...
            <el-col :span="3" style="text-align: right;">
              <el-dropdown placement="bottom">
                <el-button type="primary" size="small" icon="iconfont iconwenjianjia piliang" style="margin:0;">批量操作</el-button>
          ...
          <el-table :data="tableData" style="width: 100%;margin-bottom:12px;" border @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="realNumber" label="正常"></el-table-column>
            <el-table-column prop="silentNumber" label="沉默号"></el-table-column>
            <el-table-column prop="emptyNumber" label="空号"></el-table-column>
            <el-table-column prop="riskNumber" label="风险号"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getStatusList(ids) {
      // this.getRunningList()
      const { data } = await this.$http.post('front/empty/getStatusList', { ids })
      // if (data.code !== 200) return this.$message.error(data.msg)
      if (data.code !== 200) {
    ...
    // 获取检测记录
    async getPageList() {
      const { data } = await this.$http.post('front/empty/getPageList', this.queryInfo)
      if (data.code !== 200) return this.$message.error(data.msg)
    ...
    downloadTab(rows) {
      if (rows === 'select') {
        ...
        this.selectData.forEach(item => {
          var url = `${this.$http.defaults.baseURL}/front/customer/${item.customerId}/${item.id}/${item.name + '.zip'}`
          // console.log(url)
    ...
    downloadTxt({ id, name, customerId }, names = '') {
      var url = `${this.$http.defaults.baseURL}/front/customer/${customerId}/${id}/${names || (name + '.zip')}`
      // console.log(url)
    ...
    async deleteTab(arr) {
      if (arr.length === 0) {
        ...
        arr.map(async item => {
          const { data } = await this.$http.post(`front/empty/delete/${item.id}`)
