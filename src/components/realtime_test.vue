<template>
  <section>
    <div class="realtime-test" ref="fixedView">
      <section class="body-regin">
        <div class="center-box">
          ...
          <div if="stepIndex > 0" class="realtime-handle-regin">
            <el-progress
              v-if="stepIndex == 2"
              :text-inside="true"
              :stroke-width="18"
...
export default {
  ...
  mounted() {
    try {
      const realtimeTestingID = ss.get('realtimeTestingID')
      const realtimeTestingRows = ss.get('realtimeTestingRows')
  ...
  methods: {
          ...
          case 1: // 可关（对应等级二）
            ...
            this.$root.$emit('showCertification', {
              source: {
                url: 'empty',
                type: dom,
                balance: this.personalInfo.realtimeBalance
              },
              flag: true
            })
            break
    ...
    testingAxios() {
      ...
      this.$http
        .post('/front/realtime/realtimeCheckByFile', testForm)
        .then((res) => {
          if (res.data.code === 200) {
            ...
            ss.set('realtimeTestingID', res.data.data.code)
            ss.set('realtimeTestingRows', this.fileRows)
          } else {
            this.dialogIndex = ''
    ...
    getTestProcessMobile(type) {
      const testForm = new FormData()
      testForm.append('fileId', this.checkId || ss.get('realtimeTestingID'))
      this.$http
        .post('/front/realtime/getTestProcessMobile', testForm)
        .then((res) => {
          // console.log(res)
          // debugger
          if (res.data.code === 200) {
